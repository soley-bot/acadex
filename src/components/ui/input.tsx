...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className = "", type, state = 'default', ...props }, ref) => {
    ...
    const baseClasses = "flex h-10 w-full rounded-md bg-background px-3 py-2 text-sm text-foreground ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground disabled:cursor-not-allowed disabled:opacity-50 transition-colors"
