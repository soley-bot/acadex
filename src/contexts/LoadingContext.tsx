...
export function LoadingProvider({ children }: { children: React.ReactNode }) {
  ...
  }, [isGlobalLoading, autoRefreshEnabled, loadingStates, showAutoRefreshDialog])
  ...
  const startLoading = useCallback((id: string, options: {
    ...
    if (options.timeout) {
      const timeoutId = setTimeout(() => {
        ...
        // Use direct state update instead of stopLoading to avoid circular dependency
        setLoadingStates(prev => {
          const newStates = new Map(prev)
          newStates.delete(id)
          return newStates
        ...
        // Clear timeout
        const timeoutRef = timeoutsRef.current.get(id)
        if (timeoutRef) {
  ...
  const withLoading = useCallback(async function withLoadingFunc<T>(
    id: string, 
    promise: Promise<T>, 
    options?: {
      minDuration?: number
...
export function useSmartLoading(id?: string) {
  ...
  return {
    ...
    withLoading: (promise: Promise<any>, options?: any) =>
